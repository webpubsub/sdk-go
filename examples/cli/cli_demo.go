...
func connect() {
	...
	logfileName := "webpubsubMessaging.log"
	...
	config.Log.SetPrefix("WebPubSub :->  ")
	config.PublishKey = "pub-c-deeb6f03-8193-4a34-b253-e89f915dc7b8"   // "demo"
	config.SubscribeKey = "sub-c-e548adb7-3388-4d3a-8134-91d0130312e2" // "demo"
	...
	go func() {
		for {
			...
			case file := <-listener.File:
				fmt.Print(fmt.Sprintf("%s Subscribe Response:", outputPrefix))
				fmt.Println(" --- MessageActionsEvent: ")
				fmt.Println(fmt.Sprintf("file.File.WPSMessage.Text: %s", file.File.WPSMessage.Text))
				fmt.Println(fmt.Sprintf("file.File.WPSFile.Name: %s", file.File.WPSFile.Name))
				fmt.Println(fmt.Sprintf("file.File.WPSFile.ID: %s", file.File.WPSFile.ID))
				fmt.Println(fmt.Sprintf("file.File.WPSFile.URL: %s", file.File.WPSFile.URL))
				...
				resDLFile, statusDLFile, errDLFile := pn.DownloadFile().Channel("demo-channel").CipherKey("enigma").ID(file.File.WPSFile.ID).Name(file.File.WPSFile.Name).Execute()
...
func getMessageActionsRecursive(channel string, start string, more bool, counter int) {
	var res *webpubsub.WPSGetMessageActionsResponse
	if start == "" {
...
func printMessageActions(res *webpubsub.WPSGetMessageActionsResponse, counter int) {
	if res != nil {
...
func manageMembers(args []string) {
	...
	incl := []webpubsub.WPSChannelMembersInclude{
...
func manageMemberships(args []string) {
	...
	incl := []webpubsub.WPSMembershipsInclude{
	...
	in := webpubsub.WPSMembershipsSet{
		Channel: channel,
		Custom:  custom3,
	}
	...
	up := webpubsub.WPSMembershipsSet{
		Channel: channel,
		Custom:  custom4,
	}

	inArr := []webpubsub.WPSMembershipsSet{
		in,
		up,
	}

	re := webpubsub.WPSMembershipsRemove{
		Channel: channel,
	}

	reArr := []webpubsub.WPSMembershipsRemove{
		re,
	}
...
func granttoken(args []string) {
	...
	ch := make(map[string]webpubsub.ChannelPermissions, len(channels))
	for _, k := range channels {
		ch[k] = webpubsub.ChannelPermissions{
			Read:   true,
			Write:  true,
			Delete: false,
		}
	...
	cg := make(map[string]webpubsub.GroupPermissions, len(groups))
	for _, k := range groups {
		cg[k] = webpubsub.GroupPermissions{
			Read:   true,
			Manage: false,
		}
	}

	chPat := make(map[string]webpubsub.ChannelPermissions, len(channelsPat))
	for _, k := range channelsPat {
		chPat[k] = webpubsub.ChannelPermissions{
			Read:   true,
			Write:  true,
			Delete: true,
		}
	...
	cgPat := make(map[string]webpubsub.GroupPermissions, len(groupsPat))
	for _, k := range groupsPat {
		cgPat[k] = webpubsub.GroupPermissions{
			Read:   true,
			Manage: true,
		}
	}
...
func parseFetch(res *webpubsub.FetchResponse, status webpubsub.StatusResponse, err error) {
	...
	if status.Error == nil {
		for channel, messages := range res.Messages {
			...
			for _, messageInt := range messages {
				message := webpubsub.FetchResponseItem(messageInt)
...
func parseHistory(res *webpubsub.HistoryResponse, status webpubsub.StatusResponse, err error) {
...
func hereNowResponse(res *webpubsub.HereNowResponse, status webpubsub.StatusResponse, err error) {
...
func publishRequest(args []string) {
	...
	for _, ch := range channels {
		...
		fmt.Println(fmt.Sprintf("res: %s status: %s", res, status))
		fmt.Println(fmt.Sprintf("%s", outputSuffix))
	}
}
