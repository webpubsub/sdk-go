...
func AssertManageMembersV2(t *testing.T, checkQueryParam, testContext bool, withFilter bool, withSort bool) {
	...
	incl := []WPSChannelMembersInclude{
		WPSChannelMembersIncludeCustom,
	}
	...
	uuid := WPSChannelMembersUUID{
		ID: id0,
	}

	in := WPSChannelMembersSet{
		UUID:   uuid,
		Custom: custom,
	}

	inArr := []WPSChannelMembersSet{
		in,
	}

	re := WPSChannelMembersRemove{
		UUID: uuid,
	}

	reArr := []WPSChannelMembersRemove{
		re,
	}
	o.Set(inArr)
	o.Remove(reArr)
...
func TestManageMembersV2ResponseValueError(t *testing.T) {
	...
	_, _, err := newWPSManageMembersResponse(jsonBytes, opts, StatusResponse{})
...
func TestManageMembersV2ResponseValuePass(t *testing.T) {
	...
	r, _, err := newWPSManageMembersResponse(jsonBytes, opts, StatusResponse{})
