...
func newSetStateBuilder(webpubsub *WebPubSub) *setStateBuilder {
...
func newSetStateBuilderWithContext(webpubsub *WebPubSub, context Context) *setStateBuilder {
	builder := setStateBuilder{
		opts: &setStateOpts{
			webpubsub: webpubsub,
			ctx:       context,
...
func (b *setStateBuilder) Execute() (*SetStateResponse, StatusResponse, error) {
	...
	b.opts.webpubsub.subscriptionManager.adaptState(stateOperation)
