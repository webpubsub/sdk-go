...
func AssertSuccessSignalGet(t *testing.T, channel string, checkQueryParam bool) {
	...
	h.AssertPathsEqual(t,
		fmt.Sprintf("/signal/%s/%s/0/%s/0/%s", webpubsub.Config.PublishKey, webpubsub.Config.SubscribeKey, channel, "%7B%22one%22%3A%22hey1%22%7D"),
		path, []int{})
...
func AssertNewSignalBuilder(t *testing.T, checkQueryParam bool, testContext bool, channel string) {
	...
	o := newSignalBuilder(webpubsub)
	if testContext {
		o = newSignalBuilderWithContext(webpubsub, backgroundContext)
	}
	o.Channel(channel)
	o.Message(msgMap)
	o.usePost(true)
	...
	h.AssertPathsEqual(t,
		fmt.Sprintf("/signal/%s/%s/0/%s/0", webpubsub.Config.PublishKey, webpubsub.Config.SubscribeKey, channel),
		path, []int{})
