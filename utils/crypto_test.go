...
func TestSignSha256(t *testing.T) {
	...
	signInput := "sub-c-7ba2ac4c-4836-11e6-85a4-0619f8945a4f\npub-c-98863562-19a6-4760-bf0b-d537d1f5c582\ngrant\nchannel=asyncio-pam-FI2FCS0A&pnsdk=WebPubSub-Python-Asyncio%252F4.0.0&r=1&timestamp=1468409553&uuid=a4dbf92e-e5cb-428f-b6e6-35cce03500a2&w=1"
...
// 	v.Set("pnsdk=", "WebPubSub-Go/4.1.2")
// 	v.Set("uuid=", "myUuid")
...
// TestWebPubSubDecryption2 tests the Webpubsub Messaging API 2 decryption.
// Assumes that the input message is deserialized
// Decrypted string should match Webpubsub Messaging API 2
func TestWebPubSubDecryption2(t *testing.T) {
	assert := assert.New(t)
	...
	b, err := json.Marshal("Webpubsub Messaging API 2")
	assert.NoError(err)
	assert.Equal(string(b), decrypted)
}

// TestWebPubSubEncryption2 tests the Webpubsub Messaging API 2 encryption.
// Assumes that the input message is not serialized
// Decrypted string should match f42pIQcWZ9zbTbH8cyLwB/tdvRxjFLOYcBNMVKeHS54=
func TestWebPubSubEncryption2(t *testing.T) {
	assert := assert.New(t)

	message := "Webpubsub Messaging API 2"
	b, err := json.Marshal(message)
	assert.NoError(err)
...
// TestWebPubSubDecryption tests the Webpubsub Messaging API 1 decryption.
// Assumes that the input message is deserialized
// Decrypted string should match Webpubsub Messaging API 1
func TestWebPubSubDecryption(t *testing.T) {
	assert := assert.New(t)
	...
	b, err := json.Marshal("Webpubsub Messaging API 1")
	assert.NoError(err)
	assert.Equal(string(b), decrypted)
}

// TestWebPubSubEncryption tests the Webpubsub Messaging API 1 encryption.
// Assumes that the input message is not serialized
// Decrypted string should match f42pIQcWZ9zbTbH8cyLwByD/GsviOE0vcREIEVPARR0=
func TestWebPubSubEncryption(t *testing.T) {
	assert := assert.New(t)

	message := "Webpubsub Messaging API 1"
	b, err := json.Marshal(message)
	assert.NoError(err)
	encrypted := EncryptString("enigma", string(b), false)
...
func CreateLoggerForTests() *log.Logger {
	...
	logfileName := "webpubsubMessagingTests.log"
