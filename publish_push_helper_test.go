...
func PushPayloadCommon(t *testing.T, withContext, withAPNS, withAPNS2, withFCM, withMPNS, withCommonPayload, setAPNSAlert bool) {
	...
	o := newPublishPushHelperBuilder(webpubsub)
	if withContext {
		o = newPublishPushHelperBuilderWithContext(webpubsub, backgroundContext)
	}
	...
	apns2 := []WPSAPNS2Data{apns2One, apns2Two}
	...
	if result != nil {
		if withAPNS2 || withAPNS {
			...
			if withAPNS2 {
				resAPNS2 := result["pn_push"].([]WPSAPNS2Data)
