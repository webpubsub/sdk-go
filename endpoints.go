...
func SetPushEnvironment(q *url.Values, env WPSPushEnvironment) {
	if string(env) != "" {
		...
		q.Set("environment", string(WPSPushEnvironmentDevelopment))
	}
}
...
func defaultQuery(uuid string, telemetryManager *TelemetryManager) *url.Values {
	...
	v.Set("pnsdk", "WebPubSub-Go/"+Version)
...
func buildURL(o endpointOpts) (*url.URL, error) {
	...
	if o.config().SecretKey != "" {
		...
		if (!o.config().UsePAMV3) || ((o.operationType() == WPSPublishOperation) && (o.httpMethod() == "POST")) {
	...
	if o.operationType() == WPSPublishOperation {
		v := query.Get("meta")
		if v != "" {
	...
	if o.operationType() == WPSSetStateOperation {
		v := query.Get("state")
