...
func TestFetchMessageActionValidation(t *testing.T) {
	...
	channels := []string{"test1", "test2"}
	o := newFetchBuilder(webpubsub)
	o.Channels(channels)
	o.Reverse(false)
	o.IncludeMessageActions(true)

	_, _, e := o.Execute()
	assert.Equal("webpubsub/validation: webpubsub: Fetch Messages: Only one channel is supported when WithMessageActions is true", e.Error())
...
func AssertNewFetchBuilderContext(t *testing.T, expectedString string, channels []string) {
	o := newFetchBuilderWithContext(webpubsub, backgroundContext)
	o.Channels(channels)
	o.Reverse(false)
...
func TestFetchResponseWithCipherInterfaceWPSOtherDisabled(t *testing.T) {
	assert := assert.New(t)
...
func TestFireValidateSubscribeKey(t *testing.T) {
	...
	assert.Equal("webpubsub/validation: webpubsub: Fetch Messages: Missing Subscribe Key", opts.validate().Error())
}
...
func TestFireValidateCH(t *testing.T) {
	...
	assert.Equal("webpubsub/validation: webpubsub: Fetch Messages: Missing Channel", opts.validate().Error())
}
