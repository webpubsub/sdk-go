...
// WebPubSub client behaviour. Configuration instance contain additional set of
// properties which allow to perform precise WebPubSub client configuration.
type Config struct {
	sync.RWMutex
...
func NewConfig(uuid string) *Config {
	c := Config{
		...
		DisableWPSOtherProcessing:     false,
		WPSReconnectionPolicy:         WPSNonePolicy,
