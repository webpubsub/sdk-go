...
func TestPublishEncryptWPSOther(t *testing.T) {
	assert := assert.New(t)
...
func TestPublishEncryptWPSOtherDisable(t *testing.T) {
	assert := assert.New(t)
	pn := NewWebPubSub(NewDemoConfig())
	pn.Config.UseRandomInitializationVector = false
	pn.Config.CipherKey = "enigma"
...
func TestPublishSequenceCounter(t *testing.T) {
	...
	for i := 1; i <= MaxSequence; i++ {
		counter := opts.webpubsub.getPublishSequence()
		if counter == MaxSequence {
			assert.Equal(1, opts.webpubsub.getPublishSequence())
			break
		}
	}
}
...
func TestNewPublishResponseTimestamp(t *testing.T) {
	...
	_, _, err := newPublishResponse(jsonBytes, StatusResponse{})
	assert.Equal("webpubsub/parsing: Error unmarshalling response: {[1, Sent, \"a\"]}", err.Error())
}
...
func TestPublishValidateSubscribeKey(t *testing.T) {
	...
	assert.Equal("webpubsub/validation: webpubsub: Publish: Missing Subscribe Key", opts.validate().Error())
}
