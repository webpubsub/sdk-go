...
func iGrantATokenSpecifyingThosePermissions(ctx context.Context) error {
	...
	grantToken := cState.WebPubSub.GrantToken()
	...
	if len(aState.ChannelGroupPermissions) != 0 {
		groupPermissions := map[string]webpubsub.GroupPermissions{}
	...
	aState.ParsedToken, err = webpubsub.ParseToken(res.Data.Token)
...
func iParseTheToken(ctx context.Context) error {
	aState := getAccessState(ctx)
	parsedToken, err := webpubsub.ParseToken(aState.TokenString)
...
func iRevokeAToken(ctx context.Context) error {
	...
	_, status, err := cState.WebPubSub.RevokeToken().Token(aState.TokenString).Execute()
...
func iPublishAMessageUsingThatAuthTokenWithChannelChannel(ctx context.Context, channel string) error {
	...
	cState.WebPubSub.SetToken(aState.TokenString)
	_, status, err := cState.WebPubSub.Publish().Channel(channel).Message("Message").Execute()
	cState.statusResponse = status
	cState.err = err
