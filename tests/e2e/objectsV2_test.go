...
func ActivateWithPAMV2() *webpubsub.WebPubSub {
	pn := webpubsub.NewWebPubSub(pamConfigCopy())
	return pn
}

func RunGrantV2(pn *webpubsub.WebPubSub, users, channels []string, read, write, manage, del, create, createPattern bool) []string {
...
func SetPNV2(pn, pn2 *webpubsub.WebPubSub, tokens []string) {
...
func ObjectsCreateUpdateGetDeleteUUIDCommon(t *testing.T, withPAM, runWithoutSecretKey bool) {
	...
	incl := []webpubsub.WPSUUIDMetadataInclude{
		webpubsub.WPSUUIDMetadataIncludeCustom,
	}
...
func ObjectsCreateUpdateGetDeleteChannelCommon(t *testing.T, withPAM, runWithoutSecretKey bool) {
	...
	incl := []webpubsub.WPSChannelMetadataInclude{
		webpubsub.WPSChannelMetadataIncludeCustom,
	}
...
func ObjectsSetRemoveMembershipsCommonV2(t *testing.T, withPAM, runWithoutSecretKey bool) {
	...
	inclUUID := []webpubsub.WPSUUIDMetadataInclude{
		webpubsub.WPSUUIDMetadataIncludeCustom,
	}
	...
	inclChannel := []webpubsub.WPSChannelMetadataInclude{
		webpubsub.WPSChannelMetadataIncludeCustom,
	}
	...
	inclSm := []webpubsub.WPSChannelMembersInclude{
		webpubsub.WPSChannelMembersIncludeUUIDCustom,
		webpubsub.WPSChannelMembersIncludeUUID,
		webpubsub.WPSChannelMembersIncludeCustom,
	}
	...
	uuid := webpubsub.WPSChannelMembersUUID{
		ID: userid,
	}

	in := webpubsub.WPSChannelMembersSet{
		UUID:   uuid,
		Custom: custom3,
	}
	uuid3 := webpubsub.WPSChannelMembersUUID{
		ID: userid3,
	}

	inUser3 := webpubsub.WPSChannelMembersSet{
		UUID:   uuid3,
		Custom: custom3,
	}

	inArr := []webpubsub.WPSChannelMembersSet{
		in,
		inUser3,
	}
	...
	if !withPAM {
		...
		up := webpubsub.WPSChannelMembersSet{
			UUID:   uuid,
			Custom: custom4,
		}

		upArr := []webpubsub.WPSChannelMembersSet{
			up,
		}
	...
	inclMemberships := []webpubsub.WPSMembershipsInclude{
		webpubsub.WPSMembershipsIncludeCustom,
		webpubsub.WPSMembershipsIncludeChannel,
		webpubsub.WPSMembershipsIncludeChannelCustom,
	}
	...
	re := webpubsub.WPSChannelMembersRemove{
		UUID: uuid,
	}

	reArr := []webpubsub.WPSChannelMembersRemove{
		re,
	}
	...
	channel2 := webpubsub.WPSMembershipsChannel{
		ID: channelid2,
	}

	inMem := webpubsub.WPSMembershipsSet{
		Channel: channel2,
		Custom:  custom3,
	}

	channel3 := webpubsub.WPSMembershipsChannel{
		ID: channelid3,
	}

	inMemChannel3 := webpubsub.WPSMembershipsSet{
		Channel: channel3,
		Custom:  custom3,
	}

	inArrMem := []webpubsub.WPSMembershipsSet{
		inMem,
		inMemChannel3,
	}
	...
	upMem := webpubsub.WPSMembershipsSet{
		Channel: channel2,
		Custom:  custom5,
	}

	upArrMem := []webpubsub.WPSMembershipsSet{
		upMem,
	}
	...
	reMem := webpubsub.WPSMembershipsRemove{
		Channel: channel2,
	}

	reArrMem := []webpubsub.WPSMembershipsRemove{
		reMem,
	}
...
func ObjectsMembershipsCommonV2(t *testing.T, withPAM, runWithoutSecretKey bool) {
	...
	resAdd, stAdd, errAdd := pn.ManageChannelMembers().Channel(channelid).Sort(sortManageChannelMembers).Set(inArr).Remove([]webpubsub.WPSChannelMembersRemove{}).Include(inclSm).Limit(limit).Count(count).Execute()
	assert.Nil(errAdd)
	assert.Equal(200, stAdd.StatusCode)
	if errAdd == nil {
		sortMembers1 := false
	...
	if !withPAM {
		...
		resUp, stUp, errUp := pn.ManageChannelMembers().Channel(channelid).Sort(sort).Set(upArr).Remove([]webpubsub.WPSChannelMembersRemove{}).Include(inclSm).Limit(limit).Count(count).Execute()
		assert.Nil(errUp)
		assert.Equal(200, stUp.StatusCode)
		if errUp == nil {
			assert.True(resUp.TotalCount > 0)
	...
	resRem, stRem, errRem := pn.ManageChannelMembers().Channel(channelid).Set([]webpubsub.WPSChannelMembersSet{}).Remove(reArr).Include(inclSm).Limit(limit).Count(count).Execute()
	assert.Nil(errRem)
	assert.Equal(200, stRem.StatusCode)
	if errRem == nil {
	...
	//Add user memberships
	resManageMemAdd, stManageMemAdd, errManageMemAdd := pn.ManageMemberships().UUID(userid2).Set(inArrMem).Remove([]webpubsub.WPSMembershipsRemove{}).Include(inclMemberships).Limit(limit).Count(count).Execute()
	//fmt.Println("resManageMemAdd -->", resManageMemAdd)
	assert.Nil(errManageMemAdd)
	assert.Equal(200, stManageMemAdd.StatusCode)
	if errManageMemAdd == nil {
	...
	resManageMemUp, stManageMemUp, errManageMemUp := pn.ManageMemberships().UUID(userid2).Sort(sort).Set(upArrMem).Remove([]webpubsub.WPSMembershipsRemove{}).Include(inclMemberships).Limit(limit).Count(count).Execute()
	//fmt.Println("resManageMemUp -->", resManageMemUp)
	assert.Nil(errManageMemUp)
	assert.Equal(200, stManageMemUp.StatusCode)
	if errManageMemUp == nil {
	...
	resManageMemRem, stManageMemRem, errManageMemRem := pn.ManageMemberships().UUID(userid2).Sort(sort).Set([]webpubsub.WPSMembershipsSet{}).Remove(reArrMem).Include(inclMemberships).Limit(limit).Count(count).Execute()
	assert.Nil(errManageMemRem)
	assert.Equal(200, stManageMemRem.StatusCode)
	if errManageMemRem == nil {
...
func ObjectsListenersCommonV2(t *testing.T, withPAM, runWithoutSecretKey bool) {
	...
	pn := webpubsub.NewWebPubSub(configCopy())
	pnSub := webpubsub.NewWebPubSub(configCopy())
	...
	ExitLabel:
		for {
			...
			case userEvent := <-listener.UUIDEvent:
				...
				if (userEvent.Event == webpubsub.WPSObjectsEventRemove) && (userEvent.UUID == userid) {
					mut.Lock()
					deleteUser = true
					mut.Unlock()
				}
				if (userEvent.Event == webpubsub.WPSObjectsEventSet) && (userEvent.UUID == userid) {
					mut.Lock()
					updateUser = true
					mut.Unlock()
				}
			case channelEvent := <-listener.ChannelEvent:
				...
				if (channelEvent.Event == webpubsub.WPSObjectsEventRemove) && (channelEvent.ChannelID == channelid) {
					mut.Lock()
					deleteChannel = true
					mut.Unlock()
				}
				if (channelEvent.Event == webpubsub.WPSObjectsEventSet) && (channelEvent.ChannelID == channelid) {
					mut.Lock()
					updateChannel = true
					mut.Unlock()
				}
			...
			case membershipEvent := <-listener.MembershipEvent:
				...
				if (membershipEvent.Event == webpubsub.WPSObjectsEventSet) && (membershipEvent.ChannelID == channelid) && (membershipEvent.UUID == userid) && ((membershipEvent.Channel == channelid) || (membershipEvent.Channel == userid)) {
					mut.Lock()
				...
				if (membershipEvent.Event == webpubsub.WPSObjectsEventRemove) && (membershipEvent.ChannelID == channelid) && (membershipEvent.UUID == userid) && ((membershipEvent.Channel == channelid) || (membershipEvent.Channel == userid)) {
					mut.Lock()
	...
	_, stAdd, errAdd := pn.ManageChannelMembers().Channel(channelid).Set(inArr).Remove([]webpubsub.WPSChannelMembersRemove{}).Include(inclSm).Limit(limit).Count(count).Execute()
	assert.Nil(errAdd)
	if enableDebuggingInTests {
	...
	channel := webpubsub.WPSMembershipsChannel{
		ID: channelid,
	}
	...
	resManageMemUp, stManageMemUp, errManageMemUp := pn.ManageMemberships().UUID(userid).Set(upArrMem).Remove([]webpubsub.WPSMembershipsRemove{}).Include(inclMemberships).Limit(limit).Count(count).Execute()
	...
	_, stManageMemRem, errManageMemRem := pn.ManageMemberships().UUID(userid).Set([]webpubsub.WPSMembershipsSet{}).Remove(reArrMem).Include(inclMemberships).Limit(limit).Count(count).Execute()
	assert.Nil(errManageMemRem)
	if enableDebuggingInTests {
