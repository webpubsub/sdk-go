...
func SubscribesLogsForQueryParams(t *testing.T) {
	...
	//https://ps.pndsn.com/v1/auth/grant/sub-key/sub-c-e41d50d4-43ce-11e8-a433-9e6b275e7b64?m=1&auth=authkey1,authkey2&channel=ch1,ch2&timestamp=1535719219&pnsdk=WebPubSub-Go/4.1.3&uuid=pn-a83164fe-7ecf-42ab-ba14-d2d8e6eabd7a&r=1&w=1&signature=0SkyfvohAq8_0phVi0YhCL4c2ZRSPBVwCwQ9fANvPmM=
	...
	ExitLabel:
		for {
			...
			case status := <-listener.Status:
				...
				case webpubsub.WPSRequestMessageCountExceededCategory:
					doneSubscribe <- true
					break ExitLabel
				default:
	...
	ExitLabel:
		for {
			...
			case status := <-listener.Status:
				switch status.Category {
				case webpubsub.WPSConnectedCategory:
					//fmt.Println("WPSConnectedCategory...")
					doneSubscribe <- true
					break
				case webpubsub.WPSDisconnectedCategory:
					//fmt.Println("WPSDisconnectedCategory...")
					doneUnsubscribe <- true
					break
				case webpubsub.WPSAcknowledgmentCategory:
					doneUnsubscribe <- true
					break
				case webpubsub.WPSCancelledCategory:
					continue
				default:
...
func TestSubscribePublishUnsubscribePushHelper(t *testing.T) {
	...
	apns2One := webpubsub.WPSAPNS2Data{
		CollapseID: "invitations",
		Expiration: "2019-12-13T22:06:09Z",
		Version:    "v1",
		Targets: []webpubsub.WPSPushTarget{
			webpubsub.WPSPushTarget{
	...
	apns2Two := webpubsub.WPSAPNS2Data{
		CollapseID: "invitations",
	...
	apns2 := []webpubsub.WPSAPNS2Data{apns2One, apns2Two}

	mpns := webpubsub.WPSMPNSData{
		Title:       "title",
		Type:        "type",
		Count:       1,
		BackTitle:   "BackTitle",
	...
	fcm := webpubsub.WPSFCMData{
		Data: webpubsub.WPSFCMDataFields{
			Summary: "summary",
			Custom: map[string]interface{}{
...
func TestSubscribePublishUnsubscribeWPSOtherDisable(t *testing.T) {
	assert := assert.New(t)
	pubMessage := make(chan interface{})
...
func TestSubscribePublishUnsubscribeWPSOther(t *testing.T) {
	assert := assert.New(t)
	pubMessage := make(chan interface{})
	s := map[string]interface{}{
		"id":        1,
...
func TestSubscribePublishUnsubscribeWPSOtherComplex(t *testing.T) {
	assert := assert.New(t)
	pubMessage := make(chan interface{})
...
func TestSubscribePublishUnsubscribeInterfaceWithoutWPSOther(t *testing.T) {
	assert := assert.New(t)
	pubMessage := make(chan interface{})
	s := map[string]interface{}{
		"id":        3,
...
func TestSubscribePublishUnsubscribeInterfaceWithoutWPSOtherEnc(t *testing.T) {
	assert := assert.New(t)
	pubMessage := make(chan interface{})
	s := map[string]interface{}{
		"id":        4,
...
func TestSubscribePublishUnsubscribeWPSOtherDisablePost(t *testing.T) {
	assert := assert.New(t)
	pubMessage := make(chan interface{})
	s := map[string]interface{}{
		"id":        2,
...
func TestSubscribePublishUnsubscribeWPSOtherPost(t *testing.T) {
	assert := assert.New(t)
	pubMessage := make(chan interface{})
	s := map[string]interface{}{
		"id":        1,
...
func TestSubscribePublishUnsubscribeWPSOtherComplexPost(t *testing.T) {
	assert := assert.New(t)
	pubMessage := make(chan interface{})
	s1 := map[string]interface{}{
		"id":        1,
...
func TestSubscribePublishUnsubscribeInterfaceWithoutWPSOtherPost(t *testing.T) {
	assert := assert.New(t)
	pubMessage := make(chan interface{})
	s := map[string]interface{}{
		"id":        3,
...
func TestSubscribePublishUnsubscribeInterfaceWithoutWPSOtherEncPost(t *testing.T) {
	assert := assert.New(t)
	pubMessage := make(chan interface{})
	s := map[string]interface{}{
		"id":        4,
...
func SubscribePublishUnsubscribeMultiCommon(t *testing.T, s interface{}, cipher string, pubMessage chan interface{}, disableWPSOtherProcessing bool, usePost bool) {
	assert := assert.New(t)
	doneSubscribe := make(chan bool)
	doneUnsubscribe := make(chan bool)
	...
	pn.Config.DisableWPSOtherProcessing = disableWPSOtherProcessing
	//pn.Config.Log = log.New(os.Stdout, "", log.Ldate|log.Ltime|log.Lshortfile)
...
/*func TestSubscribePublishUnsubscribeWPSOtherDisable(t *testing.T) {
	...
	pn := webpubsub.NewWebPubSub(configCopy())
	pn.Config.CipherKey = "enigma"
	pn.Config.DisableWPSOtherProcessing = true
	pn.Config.Log = log.New(os.Stdout, "", log.Ldate|log.Ltime|log.Lshortfile)
	...
	ExitLabel:
		for {
			...
			case status := <-listener.Status:
				...
				if len(status.AffectedChannels) == 1 && status.Operation == webpubsub.WPSUnsubscribeOperation {
...
func JoinLeaveChannel(t *testing.T) {
	...
	pn := webpubsub.NewWebPubSub(configEmitter)
...
func Subscribe403Error(t *testing.T) {
	...
	pamConfig.SecretKey = ""
	pn2 := webpubsub.NewWebPubSub(pamConfig)
...
func TestSubscribeWithCustomTimetoken(t *testing.T) {
	...
	ExitLabel:
		for {
			select {
			case status := <-listener.Status:
				if status.Category == webpubsub.WPSConnectedCategory {
					doneConnected <- true
					break
				} else {
...
func TestSubscribeWithFilter(t *testing.T) {
	...
	pnPublish := webpubsub.NewWebPubSub(configCopy())
...
func ReconnectionExhaustion(t *testing.T) {
	...
	config.WPSReconnectionPolicy = webpubsub.WPSLinearPolicy
	...
	ExitLabel:
		for {
			...
			case status := <-listener.Status:
				...
				switch status.Category {
				case webpubsub.WPSReconnectionAttemptsExhausted:
					doneSubscribe <- true
				default:
					//if count > 1 {
					//errChan <- fmt.Sprintf("Non WPSReconnectedCategory event, %s", status)
					//fmt.Println(status)
					//}
				}
				count++
