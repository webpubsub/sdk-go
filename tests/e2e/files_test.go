...
func processInterface(in interface{}) {
	...
	case map[string]*webpubsub.WPSFileMessageAndDetails:

		fmt.Printf("*webpubsub.WPSFileMessageAndDetails")
	case map[string]webpubsub.WPSFileMessageAndDetails:
...
func FileUploadCommon(t *testing.T, useCipher bool, customCipher string, filepathInput, filepathOutput string) {
	...
	ExitLabel:
		for {
			...
			case file := <-listener.File:
				...
				fileDataChannel <- FileData{file.File.WPSFile.ID, file.File.WPSFile.URL, file.File.WPSFile.Name, file.File.WPSMessage.Text}
	...
	fetchCall := func() error {
		...
		for i := 0; i < len(chMessages); i++ {
			...
			if enableDebuggingInTests {
				fmt.Println("webpubsub.WPSFileDetails", file.ID)
				fmt.Println("webpubsub.WPSFileDetails", file.Name)
			}
			if msg, ok := m.(webpubsub.WPSPublishMessage); !ok {
				if enableDebuggingInTests {
					fmt.Println("!webpubsub.WPSPublishMessage")
				}
			} else {
				if enableDebuggingInTests {
					fmt.Println("webpubsub.WPSPublishMessage", msg.Text)
				}
